...
fn get_sbox() -> Matrix<u8> {
    ...
    let sbox = matrix::from_elems(16, 16, &sbox_elements);
    sbox
}

fn get_rcon() -> Matrix<u8> {
    let rcon_elements: [u8; 256] = [
    ...
    let rcon = matrix::from_elems(16, 16, &rcon_elements);
    rcon
}
...
fn matrix_row_rotate(m: &mut Matrix<u8>, row: usize, iters: usize) -> &mut Matrix<u8> {
    ...
    for _ in 0..iterations {
        // Shift the row over by one item
        let row_nums = [m.get(row,3), m.get(row,0), m.get(row,1), m.get(row,2)];
...
fn sub_bytes(state: &mut Matrix<u8>, sbox: Matrix<u8>) -> &mut Matrix<u8> {
    for i in 0..state.num_rows()  {
        for j in 0..state.num_cols()  {
            ...
            let vec = vec![byte];
            let byte_as_string = match String::from_utf8(vec) {
                Ok(v) => v,
                Err(e) => panic!("Invalid UTF-8 sequence: {}", e),
            };

            let hex_row = byte_as_string.as_bytes()[0];
            let hex_col = byte_as_string.as_bytes()[1];
